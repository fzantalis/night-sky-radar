...
// NOTE: these take `Propagator&`, not `const Propagator&`. positionAt() is
// deliberately non-const because the vendored library mutates through const, which
// made a const reference look shareable across tasks when it is not. A const& here
// will not compile.`
...
int64_t refineCrossing(Propagator& prop, const Observer& obs,
    ...
    // std::llabs, NOT `hi - lo > 1`. The set-side call passes its arguments
    // reversed (lo is chronologically LATER than hi) because the floor-side
    // invariant is what matters, not the time order. A bare `hi - lo > 1`
    // is negative there, so the loop never runs and the set time silently
    // stays on the coarse grid - unrefined, and biased short by up to one
    // COARSE_STEP_SEC. The midpoint below already handles a negative delta.
...
> **REVISED 2026-09-04 — read this before Step 1. The original Step 1 approach does not work.**
>
> The `PsramVector<Tracked>` allocator below only relocates the vector's own array.
> `Propagator` holds `std::unique_ptr<libsgp4::Tle>` and `std::unique_ptr<libsgp4::SGP4>`
> created with `std::make_unique`, so those payloads — plus several `std::string`
> allocations inside `libsgp4::Tle` — stay on the **internal** heap regardless. At ~200
> objects that is roughly 200-280 KB against about 275 KB of free internal DRAM after
> WiFi. It will not fit, and the "if free PSRAM barely moved, check the allocator is
> wired" note further down would send you hunting the wrong bug.
>
> **The allocation decision must live inside `Propagator`.** But `lib/core/` may not
> include ESP-IDF headers, so the allocator has to be injected. Create:
>
> ```cpp
> // lib/core/src/CoreAlloc.h
> #pragma once
> #include <cstddef>
>
> namespace corealloc {
...
>
> // Firmware calls this ONCE at boot with PSRAM-backed functions. Anything that
> // never calls it - native tests included - gets plain malloc/free, so the core
> // stays platform-free and testable.
...
> ```
>
> `Propagator` then allocates its two objects with `corealloc::alloc` + placement new and
> releases them with an explicit destructor call + `corealloc::free`, via a custom deleter
> on the `unique_ptr`s. Firmware calls `corealloc::setAllocator` in `setup()` with
> functions backed by `heap_caps_malloc(n, MALLOC_CAP_SPIRAM)` / `heap_caps_free`.
>
> Keep `PsramVector` for the `Tracked` vector itself — it is still correct and still
> worth having. It is simply not sufficient on its own.
>
> **The half-of-previous validation heuristic must become PER-GROUP.** M0-M1 introduced
> "reject a parse yielding fewer than half the currently-tracked count" to catch a
> truncated download. That was written when there was one group. With two, a combined
> baseline breaks it: `stations` holds ~21 element sets while the merged total is ~175, so
> `21 * 2 < 175` rejects a perfectly good `stations` body on every refresh after the first
> successful merge. Because the groups are `&&`-chained, that fails the whole refresh —
> `markFetched()` and `rebuildFromGroups()` never run, backoff pins at 60 minutes, and the
> device silently runs on ever-staler elements forever.
>
> Keep a **separate previous-count per group**, set when that group's `save()` succeeds and
> seeded from the cached parse in `begin()`. A baseline of 0 means "no history yet", so only
> the empty check applies. Also put the group name in the rejection log line.
>
> While you are there, move the count check itself into `lib/core` as a pure
> `bool tleCountIsPlausible(int count, int previousCount)` with native tests, so this guard
> is testable at all — today it lives in a file-static in `ScopeService.cpp` and nothing can
> reach it.
>
> **Verification that actually distinguishes success from failure:** log
> `ESP.getFreePsram()` and `ESP.getFreeHeap()` immediately before and after
> `rebuildFromGroups()`. Success is PSRAM falling by roughly 200 KB **while internal heap
> stays nearly flat**. If internal heap drops by that amount instead, the payloads are
> still on the wrong side and the deleter/placement-new path is not being used.
